...
words <- c("hi", "how", "are", "you", "doing", "I'm", "sleepy")
...
# "g" through "m"
g_mwords <- words[substring(words, 1, 1) >= "g" & substring(words, 1, 1) <= "m"]
...
word_bin <- function(word_vector, letter1, letter2) {
  new_vector <- words[words >= letter1 & substring(words, 1, 1) <= letter2]
  return(new_vector)
}
